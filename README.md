...
`scripts/seed.py --count 40 --seed 3` seeds the demo pool: 40 students clustered around the evening flights, with a few outliers, generated by `poolgen.py`. That pool becomes **15 cabs instead of 40**. `scripts/sweep.py --compare 0.5 2.5` shows the same 40 students grouped at two values of `upsilon` — 20 cabs with 10 people alone, against 15 cabs with 2 — which is one knob trading convenience against how many people travel alone. `scripts/show_pool.py` prints each request exactly as the solver receives it.
