...
For recording a demo: `scripts/demo_reset.sh` resets the tables, seeds the 40-student demo pool (poolgen seed 396), stops any scheduler, and checks that the demo will behave before it says `READY`, including that the new user's cab is the same two people for every sensible entry: in most pools four or five students sit near any given time, so a slightly different window swaps someone in, and seed 396 was found by search because it doesn't. Open the page with `?demo=1` to get a **Run a release now** button in the Release board and a form that opens on 7 pm ±15. The shot-by-shot script is in `arch/demo-script.md`.
...
`scripts/seed.py --count 40 --seed 3` seeds a sample pool: 40 students clustered around the evening flights, with a few outliers, generated by `poolgen.py` (that one becomes **16 cabs instead of 40**; the demo's seed 396 becomes **15**). `scripts/sweep.py --compare 0.5 2.5` shows the same 40 students grouped at two values of `upsilon`: on seed 396, 22 cabs with 13 people alone against 15 cabs with 2 (`--seeds 396`), which is one knob trading convenience against how many people travel alone. `scripts/show_pool.py` prints each request exactly as the solver receives it.
